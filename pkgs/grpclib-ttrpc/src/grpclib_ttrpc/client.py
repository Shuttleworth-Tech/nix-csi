...
from ._messages import Request, Response  # type: ignore[attr-defined]
...
async def unary_call(
    ...
    *,
    ...
    try:
        ...
        req = Request(  # type: ignore[name-defined]
            service=service,
            method=method,
            payload=payload,
        ...
        log.debug(
            'ttrpc unary_call: sending %d-byte request '
            '(header=%s payload_hex=%s)',
            len(req_bytes),
            header.hex(),
            req_bytes[:64].hex(),
        )
        ...
        log.debug('ttrpc unary_call: waiting for response (timeout=%.0fs)', response_timeout)
        ...
        log.debug(
            ...
            length, _stream_id, msg_type, resp_flags,
        )
        ...
        log.debug('ttrpc unary_call: response payload (%d bytes): %s', length, resp_bytes[:64].hex())
        ...
        return codec.decode(resp.payload, response_type)  # type: ignore[return-value]
