...
let
  ...
  baseLabels = cfg.labels // {
    "app.kubernetes.io/component" = "node";
  };
  baseMatchLabels = cfg.matchLabels // {
    "app.kubernetes.io/component" = "node";
  };
  ...
  mkNodeDaemonSet =
    system: sysPkgs:
    let
      goarch = sysPkgs.go.GOARCH;
      labels = baseLabels // {
        "nixkube/arch" = goarch;
      };
      matchLabels = baseMatchLabels // {
        "nixkube/arch" = goarch;
      };
    in
    {
      name = "nix-node-${goarch}";
      value = {
        ...
        spec = {
          ...
          template = {
            ...
            spec = {
              ...
              nodeSelector."kubernetes.io/arch" = goarch;
              tolerations = [
                {
              ...
              initContainers = lib.mkNumberedList {
                "1" = {
                  ...
                  env = lib.mkNamedList {
                    NODE_ENV.value = sysPkgs.nixkube-node-env;
                  };
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    kubernetes.resources.${cfg.namespace} =
      let
        daemonSets = lib.listToAttrs (lib.mapAttrsToList mkNodeDaemonSet csiPkgs);
      in
      {
        DaemonSet = daemonSets;
        # DNS for pods — matches ALL node pods regardless of arch
        Service.${cfg.internalServiceName} = {
          metadata.labels = baseLabels;
          spec = {
            clusterIP = "None";
            selector = baseMatchLabels;
